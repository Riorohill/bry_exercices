#include <stdio.h>
#include <openssl/x509v3.h>

int main(void){
  /*gerado com a utilidade unix xxd -i a partir de um arquivo de Certificado
  inválido encontrado em http://fm4dd.com/openssl/certexamples.htm*/
  unsigned char cert_der[] = {
    
  };
  unsigned int cert_der_len = 535;
  // fim do código gerado automaticamente
  const unsigned char *data = cert_der;
  X509 *cert = d2i_X509(NULL, &data, cert_der_len);
  if (!cert) {
  	fprintf(stderr, "Erro na leitura do Certificado\n");
  	return EXIT_FAILURE;
  }
  char *subj = X509_NAME_oneline(X509_get_subject_name(cert), NULL, 0);
  char *issuer = X509_NAME_oneline(X509_get_issuer_name(cert), NULL, 0);
  printf("Subject: %s\n", subj);
  printf("Issuer: %s\n", issuer);
  if (X509_check_ca(cert)>=1){
    printf("Certificado Valido\n");
  } else {
    printf("Certificado Invalido\n");
  }
  X509_free(cert);
}
